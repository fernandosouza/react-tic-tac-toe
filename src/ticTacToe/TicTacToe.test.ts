...
describe("Game", () => {
  ...
  it("should initialize with a empty board", () => {
    ...
    //@ts-ignore
    expect(board.filter((slot) => slot === null).length).toBe(9);
  });
  ...
  it("should pass the winner as argument to the registered callback", () => {
    ...
    expect(onGameFinishMock.mock.calls[0][0].player).toHaveProperty(
      "name",
      "A"
    );
  });
  ...
  it("should pass undefined as argument to the registered callback if no one won", () => {
    ...
    expect(onGameFinishMock).toHaveBeenCalledTimes(1);
    expect(onGameFinishMock).toHaveBeenCalledWith(undefined);
  });
